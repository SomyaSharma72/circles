...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [requests, setRequests] = useState<HelpRequest[]>(initialRequestsData as HelpRequest[]);
  const [skills, setSkills] = useState<SkillOffer[]>(initialSkillsData as SkillOffer[]);
  ...
  const updateRequest = (requestId: string, updatedData: Partial<HelpRequest>) => {
    setRequests((prev) =>
  ...
  const updateSkillOffer = (offerId: string, updatedData: Partial<SkillOffer>) => {
    setSkills((prev) =>
      prev.map((s) => (s.id === offerId ? { ...s, ...updatedData } : s))
    );
  };

  const deleteSkillOffer = (offerId: string) => {
